...
- **`Home`** — the screen-refresh schedule on one line: the time the image was actually pulled by the panel (`Écran` — Screen) and the next refresh, e.g. `14:01 ► 16:00`. The left time is the **real moment of the `GET /display`** (recomputed on every pull); the right time is the device's **next wake** — the next clock-aligned boundary, skipping one the ESP would wake within ~5 min of (mirroring the firmware, so the time matches the real next refresh). The server regenerates the buffer a few minutes before each wake. A second line shows **both tariff periods' last switches, the current period first** — e.g. `HC 15:02   HP 05:32` (off-peak since 15:02, the last peak period had started at 05:32) — driven by the ZLinky's live `PTEC` field: each time is the actual moment the meter flipped HC↔HP, as observed on the teleinfo. The left side is always the period in effect now. The line only appears once a real transition has been seen since the server started (no clock guess, no persistence — after a restart it stays absent until the next switch, and the right side stays empty until the other period's own switch has been seen).
