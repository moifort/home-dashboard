...
- **`Home`** — the screen-refresh schedule on one line: the time the image was actually pulled by the panel (`Écran` — Screen) and the next refresh, e.g. `14:01 ► 16:00`. The left time is the **real moment of the `GET /display`** (recomputed on every pull); the right time is the device's **next wake** — the next clock-aligned boundary, skipping one the ESP would wake within ~5 min of (mirroring the firmware, so the time matches the real next refresh). The server regenerates the buffer a few minutes before each wake. Below it, **one line per completed tariff window** — grouped **HC lines then HP**, the period label (`HC`/`HP`) printed once per group — shows each window as a range, e.g. `HC 23:32 ► 05:32` / `   13:00 ► 15:00` / `HP 05:32 ► 13:00` / `   17:02 ► 23:32`. The day has two off-peak and two peak windows; the panel keeps the **last two of each period**, so it shows the full day once they have all been observed. Windows are driven by the ZLinky's live `PTEC` field: a window is recorded the moment the meter flips out of it, so a multi-window-per-day off-peak schedule renders correctly **without relying on the `HC_WINDOWS` config**. A window only appears once it has completed since the server started (no clock guess, no persistence — after a restart the panel fills up over the day, window by window); the period in effect shows its last *completed* window, since the in-progress one has no end yet.
- **`Plantes`** (Plants, optional) — one compact two-line card per configured soil sensor. Line 1 (the card title): the plant's name, then — right of the name — a red **water-drop** icon when it needs watering and a red **battery** icon when its battery is low, with a **7-day soil-moisture sparkline** hugging the gutter's right edge. Line 2: three uniform label/value pairs `hum 45%  temp 27°  lum 76` — soil moisture (`hum`), temperature (`temp`) and illuminance (`lum`). Watering is flagged from the device's own `water_warning` field when present, otherwise from an optional per-plant **moisture threshold** (a trailing `:30` in the config, % floor); the low-battery flag comes from `battery_state`. Readings are instantaneous states (no integration): the latest value of each metric is kept for the day, and the sparkline is the last 7 days' moisture (a missing day leaves a gap; no backfill). See the setup section below.
...
Any Zigbee2MQTT soil sensor that reports `soil_moisture` (%), `temperature` and `illuminance` (and, on supported devices, `water_warning` / `battery_state`) can be added as a **plant**: one card in the left-gutter `Plantes` panel, under `Home`. They read the **same local MQTT broker** as the Linky core / power sensors / water (the `MQTT_*` block above) and are all declared in **one variable**, `PLANTS_SENSORS`: a `;`-separated list of `topic:Display Name[:threshold]` entries (the first `:` splits the topic from the on-screen label; an optional trailing number is the **soil-moisture floor in %**). The server subscribes to each topic (subscribe-only — battery soil sensors publish on their own schedule, so we never poll them) and keeps the day's latest reading of each metric — **no backfill**, history starts at the first connection. Each becomes one card, in declaration order. Leave the variable empty to disable them all.

The card turns a plant's `water_warning` into a red **water-drop** icon (or, when the device sends no such field, compares the soil moisture to the configured threshold) and `battery_state` (`low`/`empty`/`critical`) into a red **battery** icon. The label's slug (lowercased, accent-stripped, spaces→`-`) is the storage key.

```env
# Ficus with a 30% watering threshold + Basilic with none (relies on water_warning):
PLANTS_SENSORS=zigbee2mqtt/ficus:Ficus:30;zigbee2mqtt/basilic:Basilic
```
