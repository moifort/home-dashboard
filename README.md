...
Monitor your electricity consumption from a Linky smart meter — read locally by a **Lixee ZLinky_TIC** Zigbee dongle through Zigbee2MQTT — on an e-paper display. The dashboard shows the last 9 days of consumption with off-peak/peak breakdown and key indicators to track your savings. Optionally, it also shows **daily solar production** from an EcoFlow PowerStream, a **crypto-bot stats banner**, **configurable power sensors** (any Zigbee2MQTT/ESPHome device reporting only instantaneous power — e.g. a water-heater `Cumulus` or a washing-machine `Lave-linge` — each shown as a daily-kWh row), a **water meter daily consumption** chart (`Eau` — Water, center column above the Solar chart), **configurable plant soil sensors** (any Zigbee2MQTT soil sensor — moisture, temperature, light — each a `Plantes` (Plants) card flagging when to water), and a **UniFi network panel** (internet/Wi-Fi quality, clients and top consumers) in the bottom-right. The empty top-left gutter holds a **`Home`** panel (screen-refresh schedule), the **`Plantes`** cards and an **`Alertes`** (Alerts) status board that turns the daily trends into plain-language notes per domain, with their financial impact.
...
# The server renders the dashboard fresh on each /display pull (no cache).
...
#### Optional — Plant soil sensors

Any Zigbee2MQTT soil sensor that reports `soil_moisture` (%), `temperature` and `illuminance` (and, on supported devices, `battery_state`) can be added as a **plant**: one card in the left-gutter `Plantes` panel, under `Home`. They read the **same local MQTT broker** as the Linky core / power sensors / water (the `MQTT_*` block above) and are all declared in **one variable**, `PLANTS_SENSORS`: a `;`-separated list of `topic:Display Name:threshold` entries (the first `:` splits the topic from the rest; the trailing numeric segment is that plant's watering threshold in soil-moisture %). The server subscribes to each topic (subscribe-only — battery soil sensors publish on their own schedule, so we never poll them) and keeps the day's latest reading of each metric — **no backfill**, history starts at the first connection. Each becomes one card, in declaration order. Leave the variable empty to disable them all.

The card shows a red **water-drop** icon when a plant's soil moisture falls below its threshold, and turns `battery_state` (`low`/`empty`/`critical`) into a red **battery** icon. A plant with no trailing `:threshold` falls back to the global `PLANTS_MOISTURE_THRESHOLD` (soil moisture %); with neither it never shows a drop — the device's own `water_warning` flag is ignored. The label's slug (lowercased, accent-stripped, spaces→`-`) is the storage key.

```env
PLANTS_SENSORS=zigbee2mqtt/ficus:Ficus:30;zigbee2mqtt/basilic:Basilic:40
PLANTS_MOISTURE_THRESHOLD=30
```
...
|--------|------|-------------|
| `GET` | `/display` | EPD binary buffer (163,200 bytes), rendered fresh from the current data — for the ESP32 |
