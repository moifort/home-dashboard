...
- **`Plantes`** (Plants, optional) — one compact two-line card per configured soil sensor. Line 1 (the card title): the plant's name, then — right of the name — a red **water-drop** icon when it needs watering and a red **battery** icon when its battery is low, with a **7-day soil-moisture sparkline** hugging the gutter's right edge. Line 2: three uniform label/value pairs `hum 45%  temp 27°  lum 76` — soil moisture (`hum`), temperature (`temp`) and illuminance (`lum`). Watering is flagged from the device's own `water_warning` field; the low-battery flag comes from `battery_state`. Readings are instantaneous states (no integration): the latest value of each metric is kept for the day, and the sparkline is the last 7 days' moisture (a missing day leaves a gap; no backfill). See the setup section below.
