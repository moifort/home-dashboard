...
- **`Home`** — the screen-refresh schedule on one line: the time the image was actually pulled by the panel (`Écran` — Screen) and the next refresh, e.g. `14:01 ► 16:00`. The left time is the **real moment of the `GET /display`** (recomputed on every pull); the right time is the device's **next wake** — the next clock-aligned boundary, skipping one the ESP would wake within ~5 min of (mirroring the firmware, so the time matches the real next refresh). The server regenerates the buffer a few minutes before each wake. Below it, **one line per tariff period** shows that period's last completed window as a range — e.g. `HC 13:00 ► 15:00` / `HP 05:32 ► 13:00` — driven by the ZLinky's live `PTEC` field: a window is recorded the moment the meter flips out of it, so a multi-window-per-day off-peak schedule renders correctly **without relying on the `HC_WINDOWS` config**. Each period's line only appears once one of its windows has completed since the server started (no clock guess, no persistence — after a restart: absent → one line → both, a few hours each step). The period in effect now shows its last *completed* window, since the in-progress one has no end yet.
