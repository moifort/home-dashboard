...
- Alerts: the `Cumulus` board section + `cumulus_rise/drop` rules (in `electricity/alerts.py`) look up the sensor **named** `Cumulus` in `data["power_sensors"]` (`electricity/alerts.py:_power_sensor`).
