"""Washer MQTT transport: reads `power` from the Zigbee2MQTT smart-plug topic.

Z2M publishes power on change; we also re-request it periodically (a `get`) so
samples keep flowing during long, steady wash cycles.
"""
import json
import logging
import threading
...
class WasherMqttListener:
    ...
    def start(self):
        self._thread = threading.Thread(target=self._run, name="washer-mqtt", daemon=True)
        self._thread.start()
    ...
    def _run(self):
        while not self._stop.is_set():
            try:
                ...
                logger.warning("Washer MQTT session ended (%s), retrying in 60s", exc)
                self._stop.wait(60)
    ...
    def _connect_and_listen(self):
        ...
        def on_connect(c, userdata, flags, reason_code, properties):
            if reason_code != 0:
                logger.error("Washer MQTT connect failed: %s", reason_code)
                return
            c.subscribe(self._topic, qos=0)
            c.publish(get_topic, json.dumps({"power": ""}), qos=0)
            logger.info("Washer MQTT connected, subscribed to %s", self._topic)
