...
- **Réseau : « mois » glissant + sections nommées par SSID** — le total « données » du panneau `Réseau` n'est plus calendaire (qui retombait à ~0 le 1ᵉʳ du mois) mais **glissant sur les 30 jours complets se terminant hier** — fenêtre toujours pleine et comparable d'un jour à l'autre ; la ligne est relibellée `hier/30 j`. Les deux mini-tables de clients portent désormais le **nom réel du SSID** (`UNIFI_SSID_MAIN`/`UNIFI_SSID_IOT`, ex. `Achum` / `Iochum`) en en-tête, au lieu du générique `Perso`/`IoT` (repli sur le générique si la variable est vide).
